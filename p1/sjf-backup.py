...
def SJF_SRT(processes, t_cs, alpha, preemptive):
  ...
  #print("first io burst: %d" % processes[0].io_bursts[0]) # TODO For debug
  ...
  while(notDone(processes)):
    ...
    # Get shortest process in ready_queue
    if not ready_queue: shortest_proc = None
    else:               shortest_proc = ready_queue[0]
